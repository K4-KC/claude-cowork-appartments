...
_Can you see listings without logging in? NoBroker is known for aggressive login / app-install prompts — record what (if anything) is gated without an account: full listings? contact? filters? Does the phone require login/OTP?_
...
_For each captured column in `docs/data-schema.md`, mark where it lives: `card` (on the results card) / `detail` (detail page only) / `login` (login-walled — a gap) / `absent`._

| Column | card / detail / login / absent | Notes |
|---|---|---|
| title, locality, city, rent | | |
| bhk, area, area_basis, furnishing | | |
| deposit, maintenance, brokerage | | |
...
_Anything processing here must normalize: "₹15k", lakh/crore notation, carpet-vs-built-up area, "Immediately" for `available_from`, sq.ft vs sq.m, etc._

## NoBroker-specific
_Is `brokerage` genuinely 0 / absent? How does `posted_by` read (owner vs relationship manager)? Is the owner's contact reachable without login? Any owner/broker filter or "verified" badge?_

## Sample captured
_How many listings written to `nobroker.csv`, and any row that needed a `notes` caveat (and why)._
