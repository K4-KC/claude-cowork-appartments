...
_Can you see listings without logging in? What modal/popup interrupts, when does it fire, and can it be dismissed without an account?_
...
_(Add/split rows as needed — the point is that every captured column is accounted for.)_
...
## Sample captured
_How many listings written to `housing.csv`, and any row that needed a `notes` caveat (and why)._
