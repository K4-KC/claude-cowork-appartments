...
- **Route-selection rule (decided 2026-06-19).** Use Maps' **"Best route"** (top/first-listed) for car, applied uniformly — even though it's occasionally not the shortest distance. Transit: shortest duration shown. Apply consistently so rankings stay comparable.
- **`grocery` dropped → `supermarket` only (decided 2026-06-19).** Maps can't separate the two (same ranked list), so `supermarket_*` is the single daily-shopping POI; there is no `grocery_*`.
- **Incremental persistence (run rule).** The geo run writes **one building's row to `geo.csv` at a time, immediately** (never buffered), so the file is always the resume point and compaction can't lose captured rows — see `docs/sites/google-maps.md`.
