...
| Daily-needs POIs ×4 | `hospital_*`, `pharmacy_*`, `gym_*`, `supermarket_*` — each `_name`, `_walk_km`, `_walk_min`, `_car_km`, `_car_min` | Nearest of each category. **`grocery_*` dropped (2026-06-19)** — Maps returns the same ranked list for `grocery` and `supermarket`, so `supermarket` is the single daily-shopping POI. |
