...
- Record the actual filter values used for a run in the run's brief `data/<run-id>/BRIEF.md` so the run is reproducible — and so Cowork can read the filters directly from there on a normal (kickoff-free) run. See `docs/cowork-run.md`.
